component extends="tests.resources.AbstractQueryBuilderSpec" {
    ...
    function limit() {
        return "SELECT * FROM ""users"" OFFSET 0 ROWS FETCH NEXT 3 ROWS ONLY";
    }

    function take() {
        return "SELECT * FROM ""users"" OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY";
    }

    function offset() {
        return "SELECT * FROM ""users"" OFFSET 3 ROWS";
    }
    ...
    function forPage() {
        return "SELECT * FROM ""users"" OFFSET 30 ROWS FETCH NEXT 15 ROWS ONLY";
    }

    function forPageWithLessThanZeroValues() {
        return "SELECT * FROM ""users"" OFFSET 0 ROWS FETCH NEXT 0 ROWS ONLY";
    }
    ...
    function batchInsert() {
        return {
            sql: "INSERT INTO ""users"" (""email"", ""name"") VALUES (?, ?); INSERT INTO ""users"" (""email"", ""name"") VALUES (?, ?)",
    ...
    function insertIgnore() {
        return {
            sql: "MERGE INTO ""users"" ""qb_target"" USING (VALUES (?, ?), (?, ?)) AS ""qb_src"" ON ""qb_target"".""email"" = ""qb_src"".""email"" WHEN NOT MATCHED THEN INSERT (""email"", ""name"") VALUES (""qb_src"".""email"", ""qb_src"".""name"")",
            bindings: [ "foo", "bar", "baz", "bleh" ]
        };
    }
    ...
    function upsertWithInsertedValue() {
        return {
            sql: "MERGE INTO ""stats"" ""qb_target"" USING (VALUES (?, ?, ?), (?, ?, ?)) AS ""qb_src"" ON ""qb_target"".""postId"" = ""qb_src"".""postId"" AND ""qb_target"".""viewedDate"" = ""qb_src"".""viewedDate"" WHEN MATCHED THEN UPDATE SET ""views"" = stats.views + 1 WHEN NOT MATCHED THEN INSERT (""postId"", ""viewedDate"", ""views"") VALUES (""qb_src"".""postId"", ""qb_src"".""viewedDate"", ""qb_src"".""views"")",
            bindings: [
                1,
                "2021-09-08",
                1,
    ...
    private function getBuilder() {
        ...
        variables.grammar = getMockBox().createMock( "qb.models.Grammars.DerbyGrammar" ).init( variables.utils );
