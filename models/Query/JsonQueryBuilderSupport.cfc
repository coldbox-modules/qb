...
component {
    ...
    /**
     * Finds a JSON path separator after any known table qualifier. Table aliases
     * may themselves contain arrows, particularly aliases generated by Quick.
     */
    public numeric function getJsonPathStart( required string column ) {
        var arrowPosition = find( "->", arguments.column );
        if ( !arrowPosition ) {
            return 0;
        }

        var qualifiers = [ getAlias() ];
        var tables = [ getTableName() ];
        if ( structKeyExists( this, "getJoiningQuery" ) ) {
            tables.append( getTable() );
        }
        for ( var join in getJoins() ) {
            tables.append( join.getTable() );
        }
        for ( var table in tables ) {
            if ( isSimpleValue( table ) ) {
                var parts = getGrammar().explodeTable( table );
                qualifiers.append( parts.alias );
                qualifiers.append( parts.table );
            }
        }
        for ( var qualifier in qualifiers ) {
            if ( len( qualifier ) && compareNoCase( left( arguments.column, len( qualifier ) + 1 ), qualifier & "." ) == 0 ) {
                return find( "->", arguments.column, len( qualifier ) + 2 );
            }
        }
        if ( structKeyExists( this, "getJoiningQuery" ) ) {
            return getJoiningQuery().getJsonPathStart( arguments.column );
        }
